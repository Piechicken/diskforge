...
| Verify and automate | SHA-256, byte-for-byte compare, graphical batch recipe studio, preflight plans, per-item result review, and versioned JSON recipes | Schema v4 adds declarative `iso_edit`, `ntfs_inject`, `ext_inject`, `hfs_inject`, and `hfs_create`; all recipe writes can be previewed before execution and raw-device writes remain rejected. The visual designer covers conversion—including IMA target selection—validation, comparison, resize, injection, classic-HFS creation, extraction, and container recipes. Comparisons can optionally report only full trailing zero sectors as ignored. |
