...
> DiskForge 是独立实现，不包含或复制 WinImage 的任何代码、品牌、图标或界面资源。WinImage 官方资料将该类产品的典型能力描述为“从介质创建映像、提取文件、创建空映像、注入文件/目录、改变格式、碎片整理和批处理”等。[1]
...
所有大文件操作以块流式读写，不会将整个映像一次装入内存。WinImage 对大容量映像也强调直接对映像文件读写而不是完整驻留内存的处理方式；DiskForge 采用相同的工程原则，但为原创实现。[2]
...
| 文件属性 | FAT 映像内选中项的修改时间编辑 | **已实现**。专业版资料特别提及映像内日期修改。[3] |
...
| 启动扇区属性 | 512 字节十六进制查看/编辑、导入启动扇区、整映像备份后应用 | **已实现**。专业版资料也列出了加载和编辑启动扇区的能力。[3] |
...
WinImage 的官方介绍列出 DMF、VHD、FAT、ISO、NTFS 和 Linux 等格式/文件系统，并描述了映像创建、查看、提取、注入、格式转换和碎片整理的用途。[1] DiskForge 优先原生实现安全且可完整测试的 FAT、ISO、RAW/IMG 和固定 VHD 工作流，同时将 VHDX、VMDK、QCOW2 交由可选的标准转换器处理。不能原生编辑的文件系统会明确显示为不可浏览/不可写，避免产生破坏性误操作。
...
[1] [Gilles Vollant Software, “What is WinImage”](https://www.winimage.com/winimage.htm)

[2] [Gilles Vollant Software, “WinImage and hard disk Image”](https://www.winimage.com/wimushlp/wini1jjf.htm)

[3] [Gilles Vollant Software, “WinImage Frequently asked questions”](https://www.winimage.com/faq.htm)
