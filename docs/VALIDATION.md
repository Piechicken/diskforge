...
在当前开发检查点，完整严格命令 `QT_QPA_PLATFORM=offscreen pytest -W error` 的结果为 **409 passed, 3 skipped**，且没有警告。跳过项仍只对应可选的真实外部文件系统夹具，不影响上述合成 FAT 元数据/移动、ZIP 单映像容器、FAT 删除候选恢复、IMD、TD0 和批量映像清单回归。
