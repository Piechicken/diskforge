...
| プラットフォーム | パッケージ | 起動方法 |
|---|---|---|
| Windows x64 | `DiskForge-v0.5.0-windows-x64.zip` | 展開して `DiskForge.exe` を実行します。 |
| Linux x64 | `DiskForge-v0.5.0-linux-x64.zip` | 展開して `./DiskForge` を実行します。 |
| macOS Intel | `DiskForge-v0.5.0-macos-intel-x64.zip` | 展開し、`DiskForge.app` を Applications に移動します。 |
| macOS Apple Silicon | `DiskForge-v0.5.0-macos-arm64.zip` | 展開し、`DiskForge.app` を Applications に移動します。 |

## v0.5.0 の新機能

v0.5.0 では、編集可能な FAT イメージへのネイティブなファイルマネージャー・ドラッグ＆ドロップ注入と、イメージ内の項目を一時コピーとして抽出して他アプリケーションへドラッグするワークフローを追加しました。ディレクトリ閲覧には、並べ替え可能な詳細表示、保持されるアイコングリッド、最近使ったイメージ、安全なダブルクリック・プレビュー、ライト/ミッドナイトテーマが加わりました。グラフィカルなバッチ抽出デザイナーは JSON v3 レシピを書き出す前に連番出力名を検証し、検出された光学メディアは読み取り専用 ISO として出力します。pytest は厳格構成で警告をエラーとして扱います。
...
| 閲覧と抽出 | FAT12/16/32、ISO9660/Joliet | ツリービュー、並べ替え可能な詳細表、保持されるアイコングリッド、安全なダブルクリック・プレビュー、一括抽出、イメージ情報、MBR/GPT 検査。 |
...
| 検証と自動化 | SHA-256、グラフィカルデザイナー、JSON バッチ、監査可能なログ | デザイナーは安全な連番をプレビューし、複数ソース抽出を支援します。無人バッチは物理デバイスへの書き込みを拒否します。 |
