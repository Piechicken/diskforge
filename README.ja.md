...
| プラットフォーム | パッケージ | 起動方法 |
|---|---|---|
| Windows x64 | `DiskForge-v0.10.0-windows-x64.zip` | 展開して `DiskForge.exe` を実行します。 |
| Linux x64 | `DiskForge-v0.10.0-linux-x64.zip` | 展開して `./DiskForge` を実行します。 |
| macOS Intel | `DiskForge-v0.10.0-macos-intel-x64.zip` | 展開し、`DiskForge.app` を Applications に移動します。 |
| macOS Apple Silicon | `DiskForge-v0.10.0-macos-arm64.zip` | 展開し、`DiskForge.app` を Applications に移動します。 |
...
## v0.10.0：安全な ISO 再構築と旧式 IMG/IMA フロッピー
...
v0.10 では HFE、DC42、2MG/2IMG、APRIDISK、CopyQM、SAP、MSA、PSI、PRI、制限された 86F v2.12、FDI v2.0、JV3、DMK、UDI v1.0、標準 SCP、正規 HxC MFM、正規 PCE PFI v0 フラックスコンテナー、正規 WOZ 2.0/2.1 Apple II コンテナー、正規 A2R 3.x フラックスコンテナー、正規 G64 v0 1541 GCR コンテナー、正規 G71 v0 両面 1571 GCR コンテナーおよび正規 P64 v0 1541 NRZI パルスコンテナーに形式固有の検査を追加しました。DC42 と 2MG/2IMG は検証済みデータ領域だけをエクスポートし、APRIDISK、CopyQM、SAP、MSA、PSI、JV3 はパーサーが通常の完全な矩形レイアウトを証明した場合だけ新規 RAW を作成します。HFE、PRI、86F、FDI、DMK、正規 HxC MFM、正規 PCE PFI v0、正規 WOZ 2.0/2.1、正規 A2R 3.x、正規 G64 v0、正規 G71 v0、正規 P64 v0 はコンテナー、ビットストリームまたはフラックス構造の検査のみで、トラック、ビットストリーム、フラックスをデコードせず RAW も生成しません。これらの経路はすべて、元ファイル書き込み、汎用変換、ファイルシステムセッション、修復、デバイス、上書き、未検証の変種を拒否します。DMK はネイティブヘッダーと IDAM ディレクトリだけを検証し、FM/MFM、データマーク、CRC は解釈しません。
...
| 閲覧と抽出 | 検証済みの表示ラベルなし旧式 DOS フロッピーおよび標準サイズの `.vfd`/`.flp`/容量接尾辞付き RAW 別名を含む FAT12/16/32、保守的な FAT12/FAT16 削除済みルートファイル候補、読み取り専用 IMD、TD0、CPC DSK、D88、APRIDISK、CopyQM、SAP、MSA、PSI、DC42、2MG/2IMG、HFE、PRI、制限付き 86F、FDI、JV3、DMK、UDI v1.0、標準 SCP、正規 HxC MFM、正規 PCE PFI v0 フラックスコンテナーの検査、正規 WOZ 2.0/2.1 Apple II コンテナーの検査、正規 A2R 3.x フラックスコンテナーの検査、正規 G64 v0 1541 GCR コンテナーの検査、正規 G71 v0 両面 1571 GCR コンテナーの検査、正規 P64 v0 1541 NRZI パルスコンテナーの検査、ISO9660/Joliet、明示選択式の安全な複数イメージ ZIP コンテナー、固定 VHD データビュー、任意の NTFS/EXT/クラシック HFS/HFS+ 読み取り専用バックエンド | 通常の ZIP は、安全なルートレベルのイメージペイロードを 1～64 個含む場合だけ、自動削除されるプライベートな読み取り専用セッションに物化されます。ペイロードが一つなら直接開き、複数イメージの ZIP ではデスクトップ、CLI、または SDK で一つを明示選択する必要があります。書き込み可能または変換可能なイメージにはなりません。旧式 RAW 別名は、接尾辞と正確なバイト数の両方が従来の 512 バイト PC フロッピー形状に一致する場合だけ識別されます。可変セクター、XDF、GCR、ハードセクター、フラックスメディアは推測しません。ツリーと表は決定的なページングと並べ替えキャッシュを使用します。検証済みの MBR/GPT パーティションは常に明示的な表インデックスで選択されます。FAT は既存の編集経路を維持し、NTFS/EXT/クラシック HFS/HFS+ は読み取り専用バックエンドで正確に検証されたオフセットにのみ開かれます。ダブルクリックすると、テキスト、画像、一般的なアーカイブ、旧式パッケージ、実行ファイル、バイナリデータ用の非実行文書ワークスペースが開きます。テキストは検索、コピー保存ができ、書き込み可能な FAT 項目だけ編集してイメージへ保存し戻せます。固定 VHD はフッターを除く一時 RAW 読み取り専用ビューで開きます。 |
...
| 内容の変更 | FAT ファイル/フォルダーの注入、空ディレクトリの明示作成、削除、名前変更、ファイル/ディレクトリツリーのディレクトリ間コピーと制御済み移動、時刻変更、安全な再構築式 ISO 編集、任意の制御済み NTFS/EXT/クラシック HFS 注入 | FAT IMG と IMA は同じ編集ワークフローを共有します。空ディレクトリは親ディレクトリが既に存在する新規パスにのみ明示作成でき、上書きや暗黙の親作成は行いません。通常ファイルまたは完全なディレクトリツリーは既存ディレクトリへ上書きなしでコピーできます。コピーは元を保持し、新しい同名の対象を必要とし、対象ディレクトリが元ツリー内にある場合は拒否します。ファイルまたはディレクトリツリーはこのような対象へ移動することもできます。ディレクトリは取り消し可能なコピーを完了してから元を削除します。削除前の取消または削除失敗では手動解決のため両方の完全なツリーが残るため、ディレクトリ移動はアトミックとは表明しません。ルート、存在しないまたはディレクトリでない移動先、名前競合、読み取り専用セッション、元ツリー内の対象は変更前に拒否されます。同一ディレクトリ内の名前変更は別操作のままです。明示した FAT 削除はパス検証後に一度に一つの非ルートファイルまたはディレクトリツリーだけを削除します。これは元に戻せず、トランザクションとは表明しません。ISO 編集は常に新規イメージを出力し、内容を検証して Rock Ridge/UDF を維持します。検証済み単一初期 El Torito エントリのみ保持し、複数ブート、ハイブリッド、曖昧な構成は拒否します。`ntfsprogs`、`e2fsprogs`、または `hfsutils` が明示的に利用可能な場合、NTFS/EXT/クラシック HFS は検証済みの独立出力イメージのルートへ新しい通常ファイルだけを追加できます。元イメージ、パーティションオフセット、メタデータ、名前変更、削除、上書きは許可されません。クラシック HFS の注入は生データフォークだけを転送し、HFS+ は読み取り専用のままです。 |
...
| 検証と自動化 | SHA-256、完全操作対応グラフィカルレシピスタジオ、事前計画、項目ごとの結果確認、JSON バッチ、監査可能なログ、ディレクトリレポート | スキーマ v4 は `iso_edit`、`ntfs_inject`、`ext_inject`、`hfs_inject`、`hfs_create`、`export_listing`、FAT の `move`、`fat_mkdir`、`fat_copy`、`fat_rename`、`fat_delete`、明示パス FAT の `fat_metadata` を追加します。`export_listing` はローカルのテキスト/HTML レポートだけを作成し、明示的な読み取り専用パーティションを対象にできます。テキスト/HTML のディレクトリレポートは、閲覧可能なすべてのファイルシステムと明示的な読み取り専用パーティションに対して一つの安定した完全走査を使用します。デザイナーは変換、検証、比較、サイズ変更、注入、クラシック HFS 作成、抽出、コンテナー操作のレシピを作成、再読込、編集します。`--dry-run` は変更前に操作を確認し、無人バッチは物理デバイスへの書き込みを拒否します。 |
...
diskforge-cli mkdir-fat demo.img /DOCS  # 新しい空ディレクトリ一つ。親は既存である必要があります
diskforge-cli copy-fat demo.img /README.TXT /DOCS  # 元を保持。ファイルまたは完全なディレクトリツリー。上書きなし
diskforge-cli move-fat demo.img /README.TXT /DOCS  # /DOCS は既存である必要があります。ディレクトリツリーは取消可能なコピー後削除を使用します
diskforge-cli delete-fat demo.img /DOCS/OLD.TXT  # 明示した非ルートファイルまたはディレクトリツリー一つ。元に戻せません
diskforge-cli set-fat-metadata demo.img /README.TXT /DOCS/NOTES.TXT --hidden --modified 2024-06-15T12:34:56  # 明示した書き込み可能 FAT パスのみ
...
diskforge-cli fdi-info capture.fdi  # FDI v2.0 多層コンテナー構造
diskforge-cli jv3-info disk.jv3  # JV3 セクターコンテナー検査
diskforge-cli convert-jv3 disk.jv3 exported.img  # 証明済みの通常矩形レイアウトのみ
diskforge-cli dmk-info capture.dmk  # ネイティブ DMK ヘッダーと IDAM ディレクトリ構造
diskforge-cli udi-info capture.udi  # CRC32 検証済み大文字 UDI v1.0 MFM トラック構造のみ
diskforge-cli scp-info capture.scp  # フラックスをデコードしない標準読み取り専用 SCP 構造のみ
diskforge-cli mfm-info capture.mfm  # 正規 HxC MFM ビットストリームコンテナー構造のみ
diskforge-cli pfi-info capture.pfi  # CRC 検証済み正規 PCE PFI v0 フラックスコンテナー構造のみ
diskforge-cli woz-info disk.woz  # 正規 WOZ 2.0/2.1 Apple II コンテナー構造のみ
diskforge-cli a2r-info capture.a2r  # 正規 A2R 3.x フラックスコンテナー構造のみ
diskforge-cli d64-info disk.d64  # 検証済み標準35トラック D64 CBM DOS のディレクトリと通常ファイル連鎖
diskforge-cli list disk.d64  # CBM DOS ディレクトリを読み取り専用で一覧表示
diskforge-cli d71-info disk.d71  # 検証済み標準70トラック両面 D71 CBM DOS のディレクトリと通常ファイル連鎖
diskforge-cli list disk.d71  # 両面 CBM DOS ディレクトリを読み取り専用で一覧表示
diskforge-cli d81-info disk.d81  # 検証済み標準80トラック両面 D81 CBM DOS のディレクトリと通常ファイル連鎖
diskforge-cli list disk.d81  # D81 CBM DOS ディレクトリを読み取り専用で一覧表示
diskforge-cli g64-info disk.g64  # 正規 G64 v0 1541 GCR コンテナー構造のみ
diskforge-cli g71-info disk.g71  # 正規 G71 v0 両面 1571 GCR コンテナー構造のみ
diskforge-cli p64-info capture.p64  # CRC 検証済み正規 P64 v0 NRZI パルスコンテナー構造のみ
...
| TD0 | 文書化された CRC を確認する通常・非高度圧縮 TD0 の読み取り専用トラック/セクター検査 | 直接ファイルシステム編集は不可。完全な矩形 CHS、正確な EOF、フラグなし、物理/論理座標一致、通常データの正確な再構築を証明した場合だけ新しい RAW を厳格に出力します。 | TD0 作成、高度圧縮、インプレース変換、修復、書き込み経路は非対応。 |
| PCE PFI v0 | 読み取り専用フラックスコンテナー構造検査 | ビッグエンディアン chunk 構文、初期値ゼロの CRC-32、トラック文脈、インデックス整列、パルストークン、長さゼロの END、正確な EOF を検証します。 | フラックス/セクターをデコードせず、RAW 出力、閲覧、変換、編集、修復、書き込みは行いません。 |
| WOZ 2.0/2.1 | 読み取り専用 Apple II コンテナー構造検査 | 署名付き WOZ2 ヘッダー、任意の CRC-32、INFO v2/v3、正規 INFO/TMAP/TRKS 順序、不透明なマップ済みトラック範囲、任意の FLUX マップ整合性、制限付き UTF-8 META 構文、正確な EOF を検証します。 | WOZ1、ビットストリーム/フラックス/セクターのデコード、RAW 出力、閲覧、変換、編集、修復、書き込みは非対応です。 |
| A2R 3.x | 読み取り専用フラックスコンテナー構造検査 | 固定 A2R3 シグネチャ、先頭 INFO v1 ブロック、境界付きリトルエンディアン chunk 構文、RWCP キャプチャ項目、SLVD 解決済みトラック項目、UTF-8 META 構文、正確な EOF を検証します。 | A2R1/A2R2、フラックス/ビットストリーム/セクターのデコード、RAW 出力、閲覧、変換、編集、修復、書き込みは非対応です。 |
| D64（標準35トラック） | 読み取り専用 CBM DOS ファイルシステム検査 | 256バイトセクター、正確に174,848バイトのイメージだけを受理し、BAM の版／カウント、ディレクトリ連鎖、通常の SEQ/PRG/USR ファイル連鎖、最終セクターのバイト数を検証する。検証済みファイルは直接、または安全な ZIP の一時展開後に一覧／抽出できる。 | 40トラック／エラーマップ変種、REL/GEOS レイアウト、GCR 復号、修復、汎用変換、作成、編集、書込み、デバイス経路は提供しない。 |
| D71（標準70トラック両面） | 読み取り専用 CBM DOS ファイルシステム検査 | 256バイトセクター、正確に349,696バイトのイメージだけを受理し、両面フラグ、面0のBAM項目、面1のBAMビットマップ／カウント領域、ディレクトリ連鎖、通常のSEQ/PRG/USRファイル連鎖、最終セクターのバイト数、システム／ディレクトリ／ファイルセクターの非重複を検証する。検証済みファイルは直接、または安全なZIPの一時展開後に一覧／抽出できる。 | 40トラック／エラーマップ変種、REL/GEOSレイアウト、GCR復号、修復、汎用変換、作成、編集、書込み、デバイス経路は提供しない。 |
| D81（標準80トラック両面） | 読み取り専用 CBM DOS ファイルシステム検査 | 256バイトセクター、正確に819,200バイトのイメージだけを受理し、1581ヘッダー、40項目ずつの二つのBAM、整合するディスクID、各40ビット割当ビットマップ／カウント、正規の線形トラック40ディレクトリ、通常のSEQ/PRG/USRファイル連鎖、最終セクターのバイト数、システム／ディレクトリ／ファイルセクターの非重複を検証する。検証済みファイルは直接、または安全なZIPの一時展開後に一覧／抽出できる。 | エラーマップ変種、拡張ディレクトリ、REL/GEOS/CBMパーティション、GCR復号、修復、汎用変換、作成、編集、書込み、デバイス経路は提供しない。 |
| G64 v0 | 読み取り専用 1541 GCR コンテナー構造検査 | 固定 `GCR-1541` バージョン 0 シグネチャ、境界付きリトルエンディアンのトラック/速度表、不透明な格納トラック割り当て、固定またはマップ済み速度ゾーン、非重複、正確な EOF を検証します。 | `GCR-1571`、GCR/セクターのデコード、RAW 出力、閲覧、変換、編集、修復、書き込みは非対応です。 |
| G71 v0 | 読み取り専用の両面 1571 GCR コンテナー構造検査 | 固定 `GCR-1571` バージョン 0 シグネチャ、厳密に 168 個のハーフトラック項目、境界付きリトルエンディアンのトラック/速度表、不透明な格納トラック割り当て、固定またはマップ済み速度ゾーン、非重複、正確な EOF を検証します。 | GCR バイトは不透明です。GCR/セクターのデコード、RAW 出力、閲覧、ファイルシステムセッション、変換、編集、修復、書き込みは行いません。 |
| P64 v0 | 読み取り専用 1541 NRZI パルスコンテナー構造検査 | 固定 `P64-1541` バージョン 0 シグネチャ、定義済みフラグ、全ストリームおよびチャンク別 CRC-32、境界付き HTPx 構文、一意のハーフトラック/面座標、範囲ストリームのバイト数、空の最終 DONE、正確な EOF を検証します。 | 範囲符号化 NRZI データは不透明です。パルス/GCR/セクターのデコード、RAW 出力、閲覧、変換、編集、修復、書き込みは行いません。 |
...
| ZIP イメージコンテナー（`.zip`） | ZIP 構造と 1～64 個の検証済み候補ペイロード | 自動削除される一時物化後の読み取り/一覧/抽出/レポートのみ。複数イメージのアーカイブでは明示選択名が必要 | 作成、変換、ファイルシステム編集、アーカイブ書き込みは不可。ルートレベルで暗号化されていない Stored/Deflated の `.img`、`.ima`、`.bin`、`.dd`、`.dmf`、`.vfd`、`.flp`、容量別名、`.d64`、`.d71`、`.d81`、`.iso`、`.hfs` の各ペイロード（2 GiB 以下）は検証を要し、不安全なメンバーが一つでもあればコンテナーを拒否します。 |
| DMG | シグネチャ情報 | ネイティブ変更なし | 互換性のある外部ワークフローを使用。 |

DiskForge は、未対応の編集パスを隠したり安全でない書き込みを試みたりしません。正規 PCE PFI v0 は公開済みのビッグエンディアン chunk 境界、CRC、トラック文脈、インデックス、パルストークン構文だけを検証します。フラックスバイトは不透明に扱い、フラックス、MFM/FM、セクターをデコードせず、RAW 出力、閲覧、変換、修復、書き込みも行いません。正規 G71 v0 は固定 `GCR-1571` バージョン 0 シグネチャ、厳密に 168 個のハーフトラック項目、境界付きリトルエンディアンのトラック/速度表、不透明な格納トラック割り当て、固定またはマップ済み速度ゾーン、非重複、正確な EOF だけを検証します。GCR バイトは不透明に扱い、GCR/セクターをデコードせず、RAW 出力、閲覧、ファイルシステムセッション、変換、修復、書き込みも行いません。正規 P64 v0 は固定 `P64-1541` バージョン 0 ヘッダー、定義済みフラグ、全ストリームと各チャンクの CRC-32、境界付き HTPx 構文、一意のハーフトラック/面座標、範囲ストリームのバイト数、空の最終 DONE、正確な EOF だけを検証します。範囲符号化 NRZI データは不透明に扱い、パルス、GCR、セクターをデコードせず、RAW 出力、閲覧、変換、修復、書き込みも行いません。正規 WOZ 2.0/2.1 は署名付き WOZ2 ヘッダー、任意の CRC、INFO v2/v3、正規 INFO/TMAP/TRKS レイアウト、不透明なマップ済みトラック範囲、任意の FLUX マップ整合性、制限付き META 構文、正確な EOF だけを検証します。WOZ1 は受け付けず、ビットストリーム、フラックス、セクターをデコードせず、RAW 出力、閲覧、変換、修復、書き込みも行いません。正規 A2R 3.x は固定 A2R3 シグネチャ、先頭 INFO v1、境界付き little-endian chunk、RWCP キャプチャ、SLVD 解決済みトラック、UTF-8 META、正確な EOF だけを検証します。A2R1/A2R2 は受け付けず、フラックス、ビットストリーム、セクターをデコードせず、RAW 出力、閲覧、変換、修復、書き込みも行いません。正規 G64 v0 は固定 `GCR-1541` バージョン 0 シグネチャ、境界付きリトルエンディアンのトラック/速度表、不透明な格納トラック範囲、固定またはマップ済み速度ゾーン、非重複、正確な EOF だけを検証します。`GCR-1571` は受け付けず、GCR、セクター、ファイルシステムをデコードまたは開かず、RAW 出力、閲覧、変換、修復、書き込みも行いません。バッチイメージ棚卸しはローカルの読み取り専用レポートワークフローであり、フォレンジックスキャナーや無人の変更ではありません。既存の非シンボリックリンクディレクトリ一つだけを受け付け、リンクを無視し、既知のイメージ拡張子だけを認識し、通常ファイルは最大 10,000 件、16 GiB を超えるファイルは除外し、走査ルート外に新しい JSON/CSV/HTML レポートだけを書き出します。イメージをマウントせず、物理デバイスを検査せず、レポートを上書きせず、バッチスキーマ v4 にも入りません。IMD はフロッピーのセクターコンテナーとして検査され、RAW または書き込み可能なファイルシステムとして自動的には扱われません。固定セクター数/サイズ、連続する `1..N` 識別子、任意マップなし、通常（通常の圧縮フィルを含む）セクターデータを持つ完全な矩形 CHS レイアウトだけから、新しい RAW を出力できます。不規則ジオメトリ、欠落/削除/不良セクター、可変レイアウト、重複記録、マップ、末尾バイト、デバイス出力、上書き、IMD 書き込み、およびビットストリーム/磁束に関する主張は拒否されます。TD0 も RAW や書き込み可能なファイルシステムではなくセクターコンテナーです。通常・非高度圧縮の `TD` 記録だけを検査し、ヘッダー/コメント/トラック/セクター CRC を検証します。新しい RAW 出力には正確な EOF、フラグなしセクター、物理/論理 CHS 一致、固定ジオメトリ、および通常の raw/反復パターン/RLE データの正確な再構築も必要です。高度圧縮 `td`、複数ボリューム、CRC 失敗、フラグまたは欠落データ、混合密度、不規則ジオメトリ、出力上書き、TD0 書き込み/編集/修復、デバイス、ビットストリーム/磁束に関する主張は拒否されます。FAT 削除済みファイルの復元は、汎用フォレンジック復元ではなく限定された**候補コピー**ワークフローです。FAT12/FAT16 の固定ルートにある通常の 8.3 スロットで、正の長さが一つのクラスター以下、開始クラスターが現在空いている候補だけを受け付けます。削除済み名前の先頭文字は取得できず、候補バイトは古いか上書き済みの場合があるため、元の名前や完全性は保証しません。FAT32、サブディレクトリ、長い名前、ゼロ長または複数クラスターのチェーン、使用中クラスター、元イメージ書き込み、既存出力の上書き、デバイス復元、バッチ復元は拒否されます。通常の ZIP は限定された**読み取り専用イメージコンテナー**であり、汎用ファイルシステムや変換元ではありません。承認済み直接イメージ拡張子を持つ安全なルートレベル、非暗号化、Stored/Deflated のペイロードを 1～64 個含むことができ、各ペイロードは 2 GiB 以下です。単一ペイロードは直接開き、複数ペイロードではデスクトップ、CLI、または SDK で正確なルートレベル名を明示選択する必要があります。フォルダー、危険な名前、暗号化、未知の圧縮、空/過大/未知のペイロード、65 件以上の項目、再帰コンテナー、仮想ディスクチェーン、変換、およびすべての ZIP 書き込みは拒否されます。一時バイトは通常のクローズ、エラー、キャンセル時に削除されます。FAT の移動は一つの通常ファイルまたは完全なディレクトリツリーを既存の移動先ディレクトリへ移せます。項目の上書きや統合は行いません。ディレクトリツリーは取り消し可能なコピー後削除を用い、削除前の取消または削除失敗では両方の完全なツリーを残すため、アトミックとは表明しません。FAT メタデータバッチは、書き込み可能な FAT イメージまたは明示選択した FAT パーティション内で明記された既存項目だけに限定されます。標準の読み取り専用、隠し、システム、アーカイブビットだけを設定または解除し、呼び出し側が指定するタイムゾーンなしの FAT 作成、更新、アクセス時刻を適用できます。空要求、ルートまたは重複パス、ワイルドカード、再帰、暗黙の現在時刻、非 FAT ファイルシステム、デバイス、ACL/ADS/所有権変更、自動選択は拒否されます。バッチプレビューは書き込みを示しますが、複数の FAT ディレクトリ項目更新に全か無かのロールバックは主張しません。 仮想ディスクの変換には **Tools → Preferences** で `qemu-img` を設定してください。NTFS/EXT/HFS/HFS+ の読み取り専用閲覧にはローカルの Sleuth Kit `fls` と `icat` が必要で、任意の制御済み注入には明示設定された `ntfscp`/`ntfsls`/`ntfscat`、`debugfs`/`e2fsck`、またはクラシック HFS 専用の注入用 `hmount`/`hcopy`/`hls`、検証済み作成用 `hformat` が必要です。アプリケーションが外部ツールを黙ってダウンロード、マウント、実行することはありません。詳細は [FILESYSTEM_INJECTION.md](docs/FILESYSTEM_INJECTION.md) を参照してください。

## 品質とビルド

FAT の作成、安全なファイルとディレクトリツリーの移動、明示選択した ZIP イメージペイロードの安全な物化とクリーンアップ、保守的な FAT 削除済み候補復元、読み取り専用 IMD 検査と厳格 RAW エクスポート、読み取り専用 TD0 検査と CRC 検証済み厳格 RAW エクスポート、CLI/SDK/バッチ/デスクトップにまたがる明示 FAT メタデータ更新、読み取り専用バッチイメージ棚卸しの絞り込みと JSON/CSV/HTML レポート、編集、起動 ISO と El Torito、オリジナルテンプレートの BPB 保持とバックアップ、一時 VHD 閲覧、展開計画、ゼロ末尾レポート、ドラッグ＆ドロップ、完全なバッチレシピ編集と事前検証、文書プレビュー/検索/保存し戻し、ページングされたディレクトリ走査、七言語の完全ワークスペース、公開 API、可搬設定、タスクセンター、フォント、クロスプラットフォームの光学メディア認識、デバイス書き込み保護、FAT 再構築を自動テストしています。pytest は厳格な設定を使い、警告をエラーとして扱います。GUI はオフスクリーン環境でも検証されます。CI は Windows、Linux、macOS Intel、macOS Apple Silicon でテストを実行し、各ネイティブターゲットをパッケージ化します。タグはメタデータと照合され、既存 Release があると資産を上書きせずワークフローが停止します。
